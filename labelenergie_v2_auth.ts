...
async function handleLogin() {
  ...
  try {
    ...
    // À ce stade, les routes portail seront ajoutées ensuite.
...
async function handleSignup() {
  ...
  try {
    ...
    // Si client CRM existe : mapping direct. Sinon : création minimale dans CRM.
...
function initPrefill() {
  ...
  emailInput.addEventListener('input', () => {
    ...
    timer = window.setTimeout(async () => {
      // Si une nouvelle saisie a eu lieu pendant le debounce, on ignore.
...
function init() {
  // Pré-remplissage au signup dès la saisie email
  initPrefill();
}
